...
avb.navbar = function() {
  ...
  var searchChange = function() {
    ...
    getCurrentMonthName = function() {
      var name = '';
      var object = avb.data[avb.section + '_dates'].months;
      $.each(object, function(key, month) {
        if (month.year === avb.thisYear) {
          name = month.value;
        }
      });
      return name;
    ...
    initialize = function() {
      ...
      var months = avb.data[avb.section + '_dates'].months;
      ...
      if (!jQuery.browser.mobile) {
        ...
        $.each(months, function(key, month) {
          // render button
          $dropdownList.append(Mustache.render($('#dropdown-template').html(), { year: month.value }));
          // attach click action
          $dropdownList.find('li').last().click(function(event) {
            ...
            $dropdownLabel.html(month.value + ' <b class="caret"></b>');
            // change year
            changeYear(year);
            // close dropdown
        ...
        $dropdownLabel.html(getCurrentMonthName() + ' <b class="caret"></b>');
        $dropdown.show();
        ...
        for (var i = avb.firstYear; i <= avb.lastYear; i++) {
          var html = '<option' + ((i == avb.thisYear) ? ' selected="selected"' : ' ') + 'value="' + i + '">' + i + '</option>';
